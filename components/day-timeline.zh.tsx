...
export function DayTimeline({
  ...
  const hoverRows = hover
    ? [
        { label: "紫外线", value: String(hover.uv), band: uvBand(hover.uv) },
        { label: "体感", value: fmtTempF(hover.feelsF), band: heatBand(hover.feelsF) },
    ...
    <div>
      ...
      >
        ...
        <div className="relative mt-1 h-32">
          ...
          >
            <defs>
              <linearGradient id="dayband-zh" x1="0" y1="0" x2="1" y2="0">
                {stops.map((s, i) => (
                  <stop key={i} offset={`${s.off}%`} stopColor={s.color} />
                ))}
              </linearGradient>
              <linearGradient id="dayfade-zh" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor="#fff" stopOpacity="0.25" />
                <stop offset="100%" stopColor="#fff" stopOpacity="0.75" />
              </linearGradient>
            </defs>
            <path d={area} fill="url(#dayband-zh)" opacity="0.38" />
            <path d={area} fill="url(#dayfade-zh)" />
            <path
              d={curve}
              fill="none"
              stroke="url(#dayband-zh)"
              strokeWidth="2.25"
          ...
          {hasBest && (
            ...
            >
              ...
              <span className="whitespace-nowrap text-[11px] font-medium text-band-0">
                最佳时段 {fmtTime(hours[best.start].time.slice(11, 16))}–
